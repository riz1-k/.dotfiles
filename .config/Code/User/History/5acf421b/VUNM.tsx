          ...
          >
            ...
            <AccordionContent className='space-y-5'>
              {liveValues.myLocations?.map((_, index) => (
                <MyLocationSelect key={index} index={index} form={form} />
              ))}
        ...
        <div className='space-y-5'>
              ...
              {liveValues?.myLocations?.[index]?.locations.map(
                (_, locationIndex) => (
                  ...
                  >
                    ...
                    <CitySelector
                      countryCode={liveValues?.myLocations?.[index]?.country}
                      value={
                        liveValues?.myLocations?.[index]?.locations?.[
                          locationIndex
                        ]?.cities ?? []
          ...
          <div className='flex justify-end'>
            <Button
              ...
              size={'sm'}
              aria-hidden={
                (liveValues?.myLocations?.[index]?.locations?.length ?? 0) <= 0
              }
              className='rounded-full text-xs aria-hidden:hidden'
              onClick={() => {
                const previousLocations = form.getValues(
                  `myLocations.${index}.locations`
                );
...
function CustomerLocationSelector(props: Props & { index: number }) {
  const { index, form } = props;
  const { data: countries } = useCountryList();
  const { mutate: getStates, stateList } = useStateList();
  const liveValues = form.watch();

  const selectedCountry = liveValues.customerLocations?.[index]?.country;
        ...
        <div className='space-y-5'>
              ...
              {liveValues?.customerLocations?.[index]?.locations.map(
                (_, locationIndex) => (
                  ...
                  >
                    ...
                    <CitySelector
                      countryCode={
                        liveValues?.customerLocations?.[index]?.country
                      }
                      value={
                        liveValues?.customerLocations?.[index]?.locations?.[
                          locationIndex
          ...
          <div className='flex justify-end'>
            <Button
              ...
              variant='outline'
              aria-hidden={
                (liveValues?.customerLocations?.[index]?.locations?.length ??
                  0) <= 0
              }
              className='rounded-full text-xs aria-hidden:hidden'
              onClick={() => {
                ...
                form.trigger(`customerLocations.${index}.locations`);
              }}
            >
              + Add Location
            </Button>
