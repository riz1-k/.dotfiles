...
const imageFileSchema = z.object({
    ...
    fileSize: z.number(),
    fileType: z.enum(['image/jpg', 'image/png', 'image/jpeg', 'image/webp'], {
        errorMap: () => ({
            message: 'File type not supported, Please select JPEG, PNG or WEBP',
        }),
    }),
});
...
export default function CreateReview({ slug }: { slug: string }) {
    ...
    const { register, setValue, watch, formState } = reviewForm;

    const onSubmitReview = useMutation({
        mutationFn: async (data: ReviewType) => {
            try {
                const res = await axios.post(`${env.BACKEND_URL}/review`, data);
                ...
                <section className="min-h-[15vh] rounded-xl bg-background p-6 shadow">
                    <section className="relative flex flex-col items-center gap-5">
                        ...
                        <input
                            ...
                            onChange={async (e) => {
                                if (e.target.files) {
                                    await imageUploadrr(e.target.files);
                                }
                                e.target.value = '';
                            }}
                        />
