...
import useDraftProduct from '../../../-hooks/useDraftData';
import { Slider } from '@/components/ui/slider';

export function DraftMainInfoPage() {
  const axios = useAxios();
  const navigate = useNavigate();
  const store = useSession().currentStore;
  const productData = useDraftProduct();
  ...
  const formSubmitMutation = useMutation({
    mutationFn: async (data: TCreateProductSchema) => {
      await axios.put(`/listing/edit/${productId}`, {
        ...data,
        store: store?._id,
        draftData: data,
        nationally: undefined,
        internationally: undefined,
        online: undefined,
      });
    },
    onSuccess: async () => {
