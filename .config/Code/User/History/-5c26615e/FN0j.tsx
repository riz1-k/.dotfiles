  ...
  return (
    <Tabs value={currentTab} className='w-fit  mx-auto'>
      <TabsList className='h-auto grid grid-cols-6 p-0'>
        ...
        >
          <Link
            to={`/dashboard//stores/$id/listings`}
            params={{ id: storeId }}
        ...
        <TabsTrigger
          value='request-callbacks'
          asChild
          className='flex items-center gap-2 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground px-6 py-3'
        >
          <Link
            to={`/dashboard//stores/$id/request-callbacks`}
            search={{ page: 1, limit: 10 }}
            params={{ id: storeId }}
          >
            <Users className='h-5 w-5' />
            <span className='text-sm font-medium'>Request Callbacks</span>
          </Link>
        </TabsTrigger>
      </TabsList>
    </Tabs>
