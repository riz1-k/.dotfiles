...
import CallLinksDialog from '~/app/_components/call-links-dialog';
...
import WebsiteLinksDialog from '~/app/_components/website-links-dialog';
...
export default function StorePage({ stores, allStore }: IStore) {
  ...
  console.log('storeData', allStore);
    ...
    <main className='flex h-full flex-col gap-5 bg-card pb-32 pt-16'>
      ...
      <section className='mt-2 flex flex-col gap-8 px-5'>
        ...
        <section className='flex flex-col gap-4'>
          ...
          <Link
            aria-disabled={!allStore.isEmailsAvailable}
            href={`mailto:${stores.storeData.businessEmail}`}
            className='flex !h-12 items-center justify-center !rounded-full !bg-ring !text-primary-foreground aria-disabled:pointer-events-none aria-disabled:opacity-50'
            onClick={() => {
              onClickingEvent('Store', {
                event: 'EMAIL_CLICK',
                entityType: 'Store',
          ...
          <WebsiteLinksDialog
            ...
            websiteLinks={stores.storeData.contactInfo
              .filter((x) => x.fieldType === 'WEBSITE')
              .map((x) => x.value)}
          >
            <Button
              disabled={!allStore.isWebsiteVisitsAvailable}
              className='!h-12 !rounded-full !bg-ring !text-primary-foreground'
            >
              Website Links
            </Button>
          </WebsiteLinksDialog>
          ...
          {openRequstForm && (
            <RequestCallBackForm
              ...
              entityType={'Store'}
              isLeadsAvailable={allStore.isLeadsAvailable}
            />
          )}
