...
const topProductsData = [
  { name: "Ibuprofen", sales: 1200 },
  { name: "Acetaminophen", sales: 900 },
  { name: "Lipitor", sales: 800 },
  { name: "Aspirin", sales: 700 },
  { name: "Naproxen", sales: 600 },
];
...
export default function Dashboard() {
  ...
  const handleSort = (column: keyof (typeof salesData)[number]) => {
  ...
  const handleExport = (format: string) => {
    ...
    xlsx.writeFile(workbook, `sales.${format}`);

    //download file
    const link = document.createElement("a");
    link.href = `sales.${format}`;
    link.download = `sales.${format}`;
    link.click();
      ...
      <Card>
        ...
        <CardContent>
          <div className="mb-4 flex flex-col space-y-4 sm:flex-row sm:space-x-4 sm:space-y-0">
            ...
            <Select
              onValueChange={(value) =>
                handleSort(value as keyof (typeof salesData)[0])
              }
            >
